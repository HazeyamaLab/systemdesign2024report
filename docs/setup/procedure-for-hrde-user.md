...
3. 開発コンテナーを作成します。
    ...
    2. 表示された画面に次の情報を入力します。
        ...
        - `Git リポジトリの名前`: 今回は**`HazeyamaLab/systemdesign2024report.git`** と入力します。
...
3. VSCodeでCoderにログインします（ログインしていない場合のみ）。
    ...
    4. VSCodeに戻り、コピーしたログイン用のコードをダイアログに貼り付けて`Enter`キーを押します。ログインに成功すると、ウインドウ右下に`Welcome to Coder`と表示されます。
4. リモート開発環境上に作成した開発コンテナーに接続します。
    1. ブラウザのCoderの画面で開発コンテナーを開きます。
    2. `VS Code Desktop`ボタンをクリックし、VSCodeを起動します。すでにVSCodeを起動している場合は、ボタンをクリックした後、自分でVSCodeのウインドウに切り替えてください。
    3. VSCodeのウインドウに`'Coder' 拡張機能がこの URI を開くことを許可しますか?`と表示されることがあります。`開く`をクリックして許可してください。
    4. `Select the platform of the remote host …`と表示された場合は、`Linux`と入力して`Enter`キーを押してください。
    5. 接続に成功すると、ウインドウ左下のリモートボタンに`Dev Container: systemdesign2024 (hrde)`と表示されます。初回の接続には数分間かかることがあります。

> [!NOTE] 「4. リモート開発環境上に作成した開発コンテナーに接続します」の手順で`Could not resolve hostname …`や`存在しないパイプに書き込もうとしました`などのエラーが表示された場合は、以下の作業を実施してください。
> 
> 1. ブラウザのCoderの画面で開発コンテナーを開きます。
> 2. `Show parent apps`ボタンをクリックすると、もう一つ`VS Code Desktop`ボタンが表示されます。それをクリックしてVSCodeを起動します。
> 3. VSCodeのウインドウに`'Coder' 拡張機能がこの URI を開くことを許可しますか?`と表示されることがあります。`開く`をクリックして許可してください。
> 4. `Select the platform of the remote host …`と表示された場合は、`Linux`と入力して`Enter`キーを押してください。
> 5. しばらくすると、VSCodeのウインドウ左下のリモートボタンに`Coder: systemdesign2024/main`と表示されます。表示されたのを確認したら、メニューバーの`File`（`ファイル`）→`Close Remote Connection`（`リモート接続を閉じる`）をクリックします。その後、VSCodeのウィンドウを閉じます。
> 6. もう一度「4. リモート開発環境上に作成した開発コンテナーに接続します」の手順を実施してください。
